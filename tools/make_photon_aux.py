...
"""Build the auxiliary photon data tabulations both readers use.
...
Writes one file per tabulation into `src/endf/datafiles`:
...
`BREMX.DAT` keeps its name because `endf.incident_photon` reads it by that name,
and this branch does not change the Python reader.
"""
...
def main() -> None:
    ...
    # One file per tabulation, not one per format. The Compton profiles and the
    # density effect are unrelated measurements that only ever shared a file
    # because they shared an HDF5 container, and only one of the two has a
    # primary source to fetch. Naming each after what it holds and where it came
    # from means a reader can tell them apart without opening them.
    write(
        DATAFILES / "compton_profiles_biggs1975.txt",
        [
            ...
            f"# as distributed in the Geant4 G4EMLOW {G4EMLOW_VERSION} data set,",
            "# under doppler/.",
            "#",
            "# Generated by tools/make_photon_aux.py.",
        ]
        + compton_section(doppler),
    )
    write(
        DATAFILES / "density_effect_sternheimer1982.txt",
        [
            "# Mean excitation energy I, subshell occupancies and subshell",
            "# ionisation energies in eV. Z = 1 to 100.",
            "#",
            "# NIST ESTAR mean excitation energies, and Sternheimer, Berger and",
            "# Seltzer, At. Data Nucl. Data Tables 30 (1984) 261. Vendored from",
            "# density_effect.h5: ESTAR publishes through a web form, so there",
            "# is no archive to fetch and verify.",
            "#",
            "# Generated by tools/make_photon_aux.py.",
        ]
        + density_section(),
